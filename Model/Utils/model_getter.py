...
from ..Energy import (
    get_energy,
    get_energy_regression,
    get_explicit_bias_regression,
    get_feature_extractor,
)
from ..Proposals import (
    get_base_dist,
    get_base_dist_regression,
    get_proposal,
    get_proposal_regression,
)
from .init_proposals import (
    init_energy_to_gaussian,
    init_energy_to_gaussian_regression,
    init_proposal_to_data,
    init_proposal_to_data_regression,
)
...
def get_model(args_dict, complete_dataset, complete_masked_dataset, loader_train):
    ...
    if base_dist is None and (
        "ebm_pretraining" not in args_dict.keys()
        or args_dict["ebm_pretraining"] is None
    ):
        print("Careful, no base_dist given, the energy might not be well initialized")

    if (
        "ebm_pretraining" in args_dict.keys()
        and args_dict["ebm_pretraining"] == "standard_gaussian"
    ):
        print("Init energy to standard gaussian")
        energy = init_energy_to_gaussian(
            energy, input_size, complete_dataset.dataset_train, args_dict
        )
        print("Init energy to standard gaussian... end")

    if (
        "proposal_pretraining" in args_dict.keys()
        and args_dict["proposal_pretraining"] == "data"
    ):
        print("Init proposal")
    ...
    ebm = ImportanceWeightedEBM(
        energy=energy, proposal=proposal, base_dist=base_dist, **args_dict
    )
    print("Get EBM... end")
...
def get_model_regression(
    args_dict, complete_dataset, complete_masked_dataset, loader_train
):
    ...
    feature_extractor = get_feature_extractor(
        input_size_x=input_size_x, args_dict=args_dict
    )
    ...
    if (
        ...
        print("Init energy to standard gaussian")
        energy = init_energy_to_gaussian_regression(
    ...
    if args_dict["proposal_name"] is not None:
        print("Get proposal")
        proposal = get_proposal_regression(
            args_dict=args_dict,
            input_size_x=input_size_x_feature,
            input_size_y=input_size_y,
            dataset=[
                complete_dataset.dataset_train,
                complete_dataset.dataset_val,
    ...
    if (
        ...
        print("Init proposal")
        proposal = init_proposal_to_data_regression(
            feature_extractor=feature_extractor,
            proposal=proposal,
            input_size_x=input_size_x,
            input_size_y=input_size_y,
    ...
    # Get base_dist :
    print("Get base_dist")
    base_dist = get_base_dist_regression(
        args_dict=args_dict,
        proposal=proposal,
        input_size_x=input_size_x_feature,
        input_size_y=input_size_y,
    ...
    if args_dict["base_dist_name"] == "proposal":
        assert (
            args_dict["train_proposal"] == False
        ), "If training the proposal, the base_dist should not be the proposal"
        for param in proposal.parameters():
            param.requires_grad = False

    explicit_bias = get_explicit_bias_regression(
        args_dict=args_dict,
        input_size_x=input_size_x_feature,
    )
