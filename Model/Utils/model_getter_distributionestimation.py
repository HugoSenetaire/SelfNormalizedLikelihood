...
from ..Proposals import get_proposal, get_base_dist
...
def get_model(cfg, complete_dataset, complete_masked_dataset, loader_train):
    ...
    if cfg.proposal.proposal_name is not None:
        print("Get proposal")
        proposal = get_proposal(cfg=cfg, input_size=input_size, dataset = [complete_dataset.dataset_train, complete_dataset.dataset_val, complete_dataset.dataset_test],)
    ...
    # Get base_dist :
    print("Get base_dist")
    base_dist = get_base_dist(cfg = cfg, proposal=proposal, input_size=input_size, dataset = complete_dataset.dataset_train,)
    ...
    if cfg.energy.ebm_pretraining == 'gaussian':
        ...
        energy = init_energy_to_gaussian(energy, input_size, complete_dataset.dataset_train, cfg)
        print("Init energy to standard gaussian... end")

    if cfg.proposal_training.proposal_pretraining == 'data':
        print("Init proposal")
        proposal = init_proposal_to_data(proposal = proposal, input_size = input_size, dataloader = loader_train, cfg = cfg)
    ...
    # Get EBM :
    print("Get EBM")
    ebm = ImportanceWeightedEBM(energy = energy, proposal = proposal, base_dist = base_dist, explicit_bias=explicit_bias, nb_sample_init_bias=cfg.explicit_bias.nb_sample_init_bias)
    print("Get EBM... end")
