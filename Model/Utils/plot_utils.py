...
def plot_energy_1d_1d_regression(algo, save_dir, energy_function = None, name = 'contour_best', samples_x = [], samples_y = [], samples_title = [], step='', energy_type = True):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if energy_function is None:
...
def plot_energy_image_1d_regression(algo, save_dir,  samples_x, energy_function = None, name = 'energy_image', samples_y = [], samples_title = [], step='', energy_type = True, transform_back = None):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    range_y = torch.arange(min_y, max_y, step_size).to(dtype = algo.dtype, device = algo.device)[:100]
    ...
    for k in range(min(batch_size, 10)) :
        ...
        axs[k, 0].imshow(current_image[0].permute(1,2,0).detach().cpu().numpy(), aspect='auto', origin='upper',)
        ...
        axs[k, 1].text(argmax_energy-5*step_size, energy.max(), "{:.2f}".format(argmax_energy), rotation=0, va="bottom", ha="center", multialignment="center")
        ...
        for l, (y_sample, title_sample) in enumerate(zip(samples_y, samples_title)):
            ...
            axs[k, l+2].scatter(y_sample[k].detach().cpu().numpy(), np.ones(y_sample[k].shape)*energy.min() , c='r', alpha = 0.1)
            mean_sample_y = torch.mean(y_sample[k]).detach().cpu().numpy()
            axs[k, l+2].axvline(mean_sample_y, c='r', alpha = 0.5)
            # Add the value next to the line
            axs[k, l+2].text(mean_sample_y-5*step_size.item(), energy.mean(), "{:.2f}".format(mean_sample_y), rotation=0, va="bottom", ha="center", multialignment="center")
