...
class GaussianMixtureAdaptiveProposal(nn.Module):
    ...
    def sample(self, nb_sample = 1):
        if self.x is None or not self.training:
            ...
            if nb_sample > len(self.x) :
                ...
                x_repeat = self.x.clone().detach().flatten(1).unsqueeze(1).expand(self.x.shape[0],repeat,-1).reshape(self.x.shape[0]*repeat,-1)
            else :
                x_repeat = self.x.clone().detach().reshape(batch_size, -1)
            samples = torch.randn_like(x_repeat) * self.std + x_repeat
    ...
    def log_prob(self, samples):
        if self.x is None or not self.training:
            ...
            log_prob = -0.5 * ((samples_expanded - x_expanded) / self.std)** 2 - 0.5 * np.log(2 * np.pi) - torch.log(self.std)    
