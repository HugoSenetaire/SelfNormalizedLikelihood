...
from .ProposalForDistributionEstimation.gaussian_mixture import get_GaussianMixtureProposal
from .ProposalForDistributionEstimation.kde import get_KernelDensity
...
def get_proposal(cfg, input_size, dataset, ):
    if not isinstance(dataset, list):
        dataset = [dataset]
    dataset = torch.utils.data.ConcatDataset(dataset)
...
def get_base_dist_regression(cfg, proposal, input_size_x_feature, input_size_y, dataset,):
    if not isinstance(dataset, list):
        dataset = [dataset]
    dataset = torch.utils.data.ConcatDataset(dataset)
    ...
    if cfg.base_distribution.proposal_name == 'proposal' : 
        assert 'adaptive' not in cfg.base_distribution.proposal_name, 'Adaptive proposal should not be used as base distribution'
        return proposal

    base_dist = dic_proposals_regression[cfg.base_distribution.proposal_name]
       
    base_dist = base_dist(input_size_x_feature, input_size_y, dataset, cfg.base_distribution)


    return base_dist


def get_proposal_regression(cfg, input_size_x_feature, input_size_y, dataset,):
    if not isinstance(dataset, list):
        dataset = [dataset]
    dataset = torch.utils.data.ConcatDataset(dataset)
