...
def get_mlp_generator(input_size, cfg):
    return small_mlp_generator(
...
class NICE(nn.Module):
    ...
    def _permutate(self, tensor, neurons, inv=False):
        ...
        if not inv:
            to_perm = (
                torch.from_numpy(np.identity(len(permutation))[:, perm])
                .to(tensor.device)
                .type_as(tensor)
            )
            ...
            inv_perm = (
                torch.from_numpy(np.identity(len(permutation))[:, inv_perm])
                .to(tensor.device)
                .type_as(tensor)
            )
    ...
    def forward(self, X, inv=False, return_y=False):
        if not inv:
            ...
            if return_y:
                return (
                    -torch.tensor(dim * 0.5 * np.log(2 * np.pi), device=X.device)
                    - 0.5 * torch.sum(y**2, dim=1)
                    + jac
                    + jac_pre,
                    y,
                )
            ...
            y = self.nice1(y, type=0, inv=inv)
            return (
                self._permutate(y, X.shape[1], inv=inv),
                -torch.tensor(dim * 0.5 * np.log(2 * np.pi), device=X.device)
