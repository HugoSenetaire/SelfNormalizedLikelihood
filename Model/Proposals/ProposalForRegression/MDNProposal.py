...
class MDNProposalRegression(nn.Module):
    def __init__(self, input_size, K = 4):
        super().__init__()
        self.K = K
        self.input_size = np.prod(input_size)
        self.network = MDNProposal_Network(np.prod(input_size), K)
    ...
    def sample(self, x_feature, nb_sample = 1, ):
        ...
        means = means.view(-1, self.input_size, self.K) # (shape: (batch_size, 3, K))
        sigmas = sigmas.view(-1, self.input_size, self.K) # (shape: (batch_size, 3, K))
        q_distr = torch.distributions.normal.Normal(loc=means, scale=sigmas)
        y_samples_K = q_distr.sample(sample_shape=torch.Size([nb_sample])) # (shape: (num_samples, batch_size, 3, K))
        ...
        y_samples = torch.transpose(y_samples, 1, 0).reshape(batch_size, nb_sample, self.input_size) # (shape: (batch_size, num_samples, input_size))
    ...
    def log_prob(self, x_feature, y):
        ...
        sigmas = sigmas.view(-1, self.input_size, self.K) # (shape: (batch_size, 3, K))
        q_distr = torch.distributions.normal.Normal(loc=means, scale=sigmas)
        y_expanded = y.unsqueeze(-1).expand(-1,-1, self.K) # (shape: (batch_size, 3, K))
