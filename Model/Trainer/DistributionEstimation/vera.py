...
class VERA(AbstractDistributionEstimation):
    ...
    def __init__(
        ...
        assert self.ebm.proposal != self.ebm.base_dist, "The proposal and the base distribution should be different"
    ...
    def configure_optimizers(self):
        ...
        parameters_proposal = (
                [child.parameters() for name,child in self.ebm.proposal.named_children() if 'signa' not in name]
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        grad_e_data = torch.autograd.grad(-energy_data.sum(), x,
            create_graph=True)[0].flatten(start_dim=1).norm(2, 1)

        
        loss_total = loss_energy + self.pg_control * (grad_e_data ** 2. / 2.).mean()
