...
class SelfNormalizedTrainer(AbstractDistributionEstimation):
    ...
    def training_energy(self, x):
        ...
        if (
            self.cfg.optim_f_theta.pg_control_mix is not None
            and self.cfg.optim_f_theta.pg_control_mix > 0
        ):
            ...
            loss_grad_estimate_mix = self.gradient_control_l2(
                aux_2, -f_theta_gen_2, self.cfg.optim_f_theta.pg_control_mix
            )
        ...
        if (
            self.cfg.optim_f_theta.clip_grad_norm is not None
            and self.cfg.optim_f_theta.clip_grad_norm > 0
        ):
