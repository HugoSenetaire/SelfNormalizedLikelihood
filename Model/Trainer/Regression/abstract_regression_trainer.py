...
from ...Utils.plot_utils import (
    plot_energy_1d_1d_regression,
    plot_energy_image_1d_regression,
)
...
class AbstractRegression(pl.LightningModule):
    ...
    def proposal_step(
        ...
        if self.train_proposal:
            ...
            proposal_loss = self.proposal_loss(
                log_prob_proposal=log_prob_proposal_data, log_estimate_z=estimate_log_z
            )
    ...
    def validation_step(self, batch, batch_idx, name="val"):
        ...
        log_prob_proposal_data = self.ebm.log_prob_proposal(
            x, y, x_feature=x_feature
        ).reshape(
            x.shape[0],
        )
        ...
        estimate_log_z, dic = self.ebm.estimate_log_z(
            x, nb_sample=num_samples, x_feature=x_feature
        )
        estimate_log_z = estimate_log_z.reshape(
            x.shape[0],
        )
        dic_output.update(dic)
        dic_output["loss_self_normalized"] = (
            energy_data + estimate_log_z.exp() - 1
        ).reshape(x.shape[0])
        dic_output["log_likelihood_self_normalized"] = -dic_output[
            "loss_self_normalized"
        ].reshape(x.shape[0])
        dic_output["loss_importance"] = (energy_data + estimate_log_z).reshape(
            x.shape[0]
        )
        dic_output["log_likelihood_importance"] = -dic_output[
            "loss_importance"
        ].reshape(x.shape[0])
        dic_output["log_likelihood_proposal"] = log_prob_proposal_data.reshape(
            x.shape[0]
        )
        ...
        if self.ebm.proposal is not None:
            if self.input_type_y != "other":
                if self.input_type_x == "images":
                    self.example_proposal_x = self.example_x_default.to(
                        dtype=self.dtype, device=self.device
                    )
                    self.example_proposal_y = (
                        self.ebm.sample_proposal(self.example_proposal_x, 100)
                        .reshape(-1, 100, np.prod(self.cfg.dataset.input_size_y))
                        .to(dtype=self.dtype, device=self.device)
                    )
                    self.example_proposal_x = (
                        self.example_proposal_x.unsqueeze(1)
                        .expand(-1, 100, *self.cfg.dataset.input_size_x)
                        .reshape(-1, 100, *self.cfg.dataset.input_size_x)
                        .to(dtype=self.dtype)
                    )
                    self.min_y, self.max_y = min(
                        torch.min(self.example_proposal_y), self.min_y_original
                    ), max(torch.max(self.example_proposal_y), self.max_y_original)
                elif self.input_type_x == "1d":
                    self.example_proposal_x = (
                        torch.arange(
                            self.min_x_original,
                            self.max_x_original,
                            (self.max_x_original - self.min_x_original) / 100,
                        )
                        .reshape(-1, 1)
                        .to(dtype=self.dtype, device=self.device)
                    )
    ...
    def initialize_examples(self, complete_dataset):
        ...
        if complete_dataset is not None:
            ...
            indexes_to_print = np.random.choice(
                len(complete_dataset.dataset_train),
                min(nb_samples_max, len(complete_dataset.dataset_train)),
                replace=False,
            )
            self.example_x = (
                complete_dataset.dataset_train.__getitem__(indexes_to_print)["data"]
                .reshape(-1, *self.cfg.dataset.input_size_x)
                .to(dtype=self.dtype)
            )
            self.example_x_default = self.example_x.clone()
            self.example_x = self.example_x.unsqueeze(1).expand(
                -1, 100, *self.cfg.dataset.input_size_x
            )
            self.example_y = (
                complete_dataset.dataset_train.__getitem__(indexes_to_print)["target"]
                .reshape(-1, np.prod(self.cfg.dataset.input_size_y))
                .unsqueeze(1)
                .expand(-1, 100, *self.cfg.dataset.input_size_y)
            )
            self.example_y = self.example_y.to(dtype=self.dtype)
            if self.input_type_x == "1d":
                self.min_x_original, self.max_x_original = min(
                    torch.min(self.example_x), self.min_x_original
                ), max(torch.max(self.example_x), self.max_x_original)
            if self.input_type_y == "1d" or self.input_type_y == "2d":
                self.min_y_original, self.max_y_original = min(
                    torch.min(self.example_y), self.min_y_original
                ), max(torch.max(self.example_y), self.max_y_original)
        else:
    ...
    def proposal_visualization(self, step=""):
        ...
        if self.ebm.proposal is not None:
            ...
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            if (
                np.prod(self.cfg.dataset.input_size_x) == 1
                and np.prod(self.cfg.dataset.input_size_y) == 1
            ):
                plot_energy_1d_1d_regression(
                    self,
                    save_dir=save_dir,
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.train_proposal:
            self.resample_proposal()
            self.proposal_visualization(step=self.current_step)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.test_name is None:
            return self.validation_step(
                batch, batch_idx, name="test"
            )  # Regular testing
        else:
            return self.validation_step(
                batch, batch_idx, name=self.test_name
            )  # Testing with a specific name for specific proposal evaluation
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.test_name is None:
            ...
            self.update_dic_logger(
                outputs, name=self.test_name + "_"
            )  # Testing with a specific name for specific proposal evaluation
    ...
    def plot_energy(
        ...
        if self.current_step - self.last_save > self.cfg.train.save_energy_every:
            ...
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            ebm_function_list = [
                lambda x, y: self.ebm.calculate_energy(x, y)[1]["f_theta"],
            ]
            ebm_function_name = [
                "f_theta",
            ]
            if (
                hasattr(self.ebm, "explicit_bias")
                and self.ebm.explicit_bias is not None
            ):
                ebm_function_list.append(
