...
class DualNCETrainer(DualAbstractDistributionEstimation):
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        energy_data, dic_output = self.ebm.calculate_energy(x_feature,)
        energy_data = energy_data.reshape(x_feature.shape[0],)
        log_prob_proposal_data = self.ebm.proposal.log_prob(x_feature).reshape(x_feature.shape[0],)


        samples = self.ebm.sample(nb_sample).to(x_feature.device, x_feature.dtype)
        ...
        if self.ebm.base_dist is not None :
            base_dist_log_prob = self.ebm.base_dist.log_prob(samples).view(samples.size(0), -1).sum(1).unsqueeze(1)
        else :
            base_dist_log_prob = torch.zeros_like(energy_samples).detach()
        energy_samples = energy_samples - base_dist_log_prob
        log_prob_proposal_samples = self.ebm.proposal.log_prob(samples).reshape(samples.shape[0], -1).sum(1).unsqueeze(1)
