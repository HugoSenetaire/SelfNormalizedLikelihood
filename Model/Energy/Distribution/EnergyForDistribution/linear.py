...
def get_fc_energy(input_size, cfg, ):
    return fc_energy(input_size,
                        cfg.dims,
                        cfg.activation,
                        cfg.last_layer_bias,
                        )
...
def get_fc_energy_sn(input_size, cfg):
    return fc_energy_sn(input_size,
                        cfg.dims,
                        cfg.activation,
                        cfg.last_layer_bias,
                        )
...
class fc_energy(nn.Module):
    def __init__(self, input_size = (1,10), dims = [100, 100, 100], activation = None, last_layer_bias = True,) -> None:
        super().__init__()
...
class fc_energy_sn(nn.Module):
    def __init__(self, input_size = (1,10), dims = [100, 100, 100], activation = None, last_layer_bias = True,) -> None:
        super().__init__()
        self.input_size = input_size
        self.linear = [spectral_norm(nn.Linear(np.prod(input_size), dims[0])),]
        for dim_in, dim_out in zip(dims[:-1],dims[1:]):
            self.linear.append(nn.ReLU())
